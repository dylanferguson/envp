...
func TestRevoke(t *testing.T) {
	...
	wrong := HashDeleteToken([]byte("wrong-token-bytes-not-32-chars!!"))
	if err := s.Revoke(t.Context(), created.ID, wrong); !errors.Is(err, ErrNotFound) {
		t.Fatalf("wrong hash: %v", err)
	}
	if err := s.Revoke(t.Context(), created.ID, HashDeleteToken(created.DeleteToken)); err != nil {
